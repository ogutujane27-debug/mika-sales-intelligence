...
# --- VIEW 3: ASK MIKA CHATBOT ENGINE WITH FIXED INDENTATION NO-LOOP DISPLAY ---
if page == "💬 Ask MIKA Market Chatbot":
    st.subheader(text[lang]["chat_header"])
    st.write(text[lang]["chat_desc"])
    
    # Kila mstari upo indented kwa nafasi 4 pekee, hakuna loop tata ya kuleta makosa mtandaoni
    if st.session_state["chat_history"]:
        st.write(st.session_state["chat_history"])
                
    user_query = st.st.text_input(text[lang]["chat_ph"], key="chatbot_input_box")
    if user_query:
