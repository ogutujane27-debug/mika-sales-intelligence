        ...
        with st.spinner("Streaming calculations from AI core..."):
            try:
                ...
                if "request_too_large" in str(e) or "413" in str(e):
                    ...
                    st.error(
                        f"⚠️ The AI model `{GROQ_MODEL}` isn't available on this Groq "
                        "account/tier right now. Check console.groq.com for which models "
                        "your account can use, then update GROQ_MODEL in the code."
                    )
                else:
            ...
            if not groq_api_key:
                ...
                try:
                    ...
                    if "request_too_large" in str(e) or "413" in str(e):
                        ...
                        reply = (
                            f"⚠️ The AI model `{GROQ_MODEL}` isn't available on this Groq "
                            "account/tier right now. Check console.groq.com for which "
                            "models your account can use, then update GROQ_MODEL in the code."
                        )
                    else:
