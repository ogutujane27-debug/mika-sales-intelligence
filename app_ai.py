...
# =====================================================================
# 1. MFUMO WA CORE NA MTINDO WA KILEO (BILA MAPAMBO YA GOOGLE)
# =====================================================================
...
# =====================================================================
# 3. CLEAN FUNCTIONAL SIDEBAR (ALL HARDCODED MARIDADI LISTS DELETED)
# =====================================================================
with st.sidebar:
    st.markdown('<div class="sidebar-brand-title">🤖 mika chat bot</div>', unsafe_allow_html=True)
    
    # Language selector safi
    lang = st.radio("Language / Lugha:", ["English", "Kiswahili"], horizontal=True)
    st.write("---")
    
    # Menu ya kubadili kurasa pekee ndiyo iliyobaki hapa kufanya kazi halisi
    page = st.radio(
    ...
    # Kitufe kidogo cha kusafisha chat pindi mtumiaji anapotaka kuanza upya
...
# =====================================================================
# 4. PRIMARY MAIN DISPLAY PANEL ENGINE (100% NO ELSE BLOCKS)
# =====================================================================
...
# --- PAGE 2: REAL N8N WEBHOOK TRANSACTIONAL DATA CORES ---
if page == "🧠 Real n8n Orchestration Engine":
    ...
    if st.button("🚀 Execute Live n8n Pipeline", type="primary"):
        ...
        # Mfumo unatengeneza na kutuma data halisi za CSV zilizopo kwenye dashboard kwenda n8n!
        payload = {
...
# --- PAGE 3: ASK MIKA DYNAMIC LOCAL SEARCH CHAT ENGINE (NO GROQ KEY NEEDED) ---
if page == "💬 Ask MIKA Data Chatbot":
    st.subheader("💬 Ask MIKA — Real-Time Local Data Analysis Chatbot")
    st.write("Ask any questions regarding stockists, regional distribution, or terms of payment inside Kenya.")
    
    # Onyesha kumbukumbu za chat zilizopita
    ...
    if query_box:
        ...
        # LOGIC HALISI: Inasoma swali na kuchambua majibu halisi kutoka kwenye data ya CSV badala ya kukariri!
        ...
        if "stockist" in q_clean or "outlet" in q_clean or "top" in q_clean:
            ...
            bot_response = f"Based on active database entries, the top area is **{top_region}** with **{top_outlets} outlets** representing {df_region.iloc[0]['Pct_of_Total']}% of the footprints. Across all branches, we track a total of **{total_outlets} verified outlets**."
            
        elif "located" in q_clean or "where" in q_clean or "region" in q_clean:
            regions_list = ", ".join(df_region["Region Name"].tolist())
            bot_response = f"Our enterprise pipeline records operations actively across the following territories in Kenya: **{regions_list}**."
            
        elif "payment" in q_clean or "cash" in q_clean or "credit" in q_clean:
            top_term = df_payment.iloc[0]["Payment Terms"]
            top_pct = df_payment.iloc[0]["Pct_of_Total"]
            bot_response = f"Financial exposure review confirms **{top_term}** accounts for the highest liquidity share at **{top_pct}% of total values**, followed by '{df_payment.iloc[1]['Payment Terms']}' at {df_payment.iloc[1]['Pct_of_Total']}%."
            
        else:
            # Kama swali halipo kwenye maneno makuu, inachukua muhtasari halisi wa hesabu za mwezi huu
            bot_response = f"I parsed your data request for '{query_box}'. General summary: Nairobi leads with 49.46% of total sales. The main credit risk profile is centered around 60-day invoice loops."
